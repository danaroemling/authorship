...
# Get word frequencies for SAT and DAI
...
ggplot(SAT_top_n_words, aes(x = reorder(word, freq), y = freq)) +
  ...
  labs(title = "Top 20 Most Frequent Words (SAT)", x = "Word", y = "Normalised Frequency per Million Words")
...
ggplot(DAI_top_n_words, aes(x = reorder(word, freq), y = freq)) +
  ...
  coord_flip() +
  scale_y_continuous(labels = label_number(scale = 1, suffix = "", accuracy = 0.01)) + 
  labs(title = "Top 20 Most Frequent Words (DAI)", x = "Word", y = "Normalised Frequency per Million Words")
```
...
# Add a column for the author (to distinguish between SAT and DAI)
...
If you are on a Mac, you likely need to install X11 as well for it to work. See here for details: https://github.com/computationalstylistics/stylo.
...
loadc <- function(paths, text_l = 500)  # Define function with paths to corpora and a minimum text length
{
  corpus <- list()       # Initialise empty list to hold parsed texts
  df_rows <- list()      # Initialise empty list to hold metadata rows
  
  for (i in 1:length(paths))  # Loop over each path
  {
    texts <- load.corpus.and.parse(file = "all",  # Initially parse all texts in the directory
                                   paths[i])
    
    texts <- list.files(paths[i])[lengths(texts) >= text_l]  # Filter out files shorter than text_l
    
    texts <- load.corpus.and.parse(file = texts,            # Re-parse only texts that meet length requirement
                                   corpus.dir = paths[i], 
                                   sample.size=text_l, 
                                   sampling="normal.sampling")
    
    aut  <- sub("\\./Data/(\\w+)/\\w+", "\\1", paths[i])  # Extract author name from file path
    reg  <- sub("\\./Data/\\w+/(\\w+)", "\\1", paths[i])  # Extract register (genre/domain) from path
    cou  <- length(texts)                                 # Count number of valid texts
    
    df_rows[[i]] <- data.frame(author = aut, register = reg, count=cou)  # Store metadata
    
    corpus[[i]] <-  texts  # Store parsed texts
  }
  
  meta <- do.call(rbind, df_rows)  # Combine metadata rows into a single data frame
  
  alltexts <- unlist(corpus, recursive = FALSE)  # Flatten corpus into a single list of texts
  mfw <- make.frequency.list(alltexts, value = FALSE, relative = FALSE)  # Create frequency list
  
  return(list(meta = meta, corpus = corpus, mfw = mfw))  # Return list containing metadata, corpus, and frequency list
}
```
...
PCA stands for Principal Component Analysis. It is a statistical method used to simplify complex data by reducing the number of variables while retaining most of the important information. Essentially, it transforms the original data into a new set of variables, called principal components, which are uncorrelated and ordered by the amount of variance they explain. 

```{r stylo_pca}
stylo.pca <- function(corpora, types)  # Define function to perform PCA on selected corpora and number of types (words)
{
  texts <- unlist(data$corpus[corpora], recursive = FALSE)  # Extract and flatten selected texts from global 'data$corpus'
  
  freq <- make.table.of.frequencies(texts,               # Create frequency table from selected texts
                                    data$mfw[1:types],   # Use top 'types' most frequent words from global 'data$mfw'
                                    absent.sensitive = TRUE,  # Include zeroes for absent words
                                    relative = TRUE)          # Use relative frequencies
  
  pca <- prcomp(freq, scale = TRUE)  # Perform PCA on frequency table, scaling the variables
  
  scores <- as.data.frame(pca$x)          # Extract PCA scores (coordinates of texts in PC space)
  scores$labels <- names(texts)           # Add text labels (file names or IDs)
  scores$authors <- sub("_.+", "", names(texts))  # Extract author names from labels (assuming format "author_filename")
  
  return(list(freq = freq, pca = pca, scores = scores))  # Return list of frequency table, PCA object, and scores data frame
}
```
...
plot.pca <- function(scores, d1, d2, cex = 0.5, cols = NULL) {
  if (!is.null(cols)) {
    col_vector <- cols[scores$authors]  # Use provided color mapping for each author
  } else {
    default_cols <- c("orangered", "forestgreen", "purple2", "goldenrod",   # Default color palette
                      "turquoise3", "darkorange4", "dodgerblue3", "orchid4", "limegreen")
    
    author_levels <- unique(scores$authors)  # Get unique author names
    author_colors <- setNames(default_cols[seq_along(author_levels)], author_levels)  # Assign colors to authors
    col_vector <- author_colors[scores$authors]  # Map colors to authors in 'scores'
  }

  x <- unlist(scores[d1])  # Extract PCA dimension for x-axis (e.g. PC1)
  y <- unlist(scores[d2])  # Extract PCA dimension for y-axis (e.g. PC2)
  labs <- rownames(scores)  # Get labels for each point (e.g. filenames or IDs)

  plot(x, y, type = "n")  # Initialize empty plot without points
  text(x, y, labels = labs, cex = cex, col = col_vector)  # Add text labels with specified size and color
}

```

This also plots, but with dots instead of file names.
Code is the same, minus the labels.
